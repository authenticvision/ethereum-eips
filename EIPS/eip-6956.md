...
NFTs are nowadays often confused as being assets themselves. Very commonly people treat an NFT's metadata (images, traits, ...) as asset-class, with their rarity often directly defining the value of an invididual NFT.
It is a common misconception from NFT-investors that metadata is immutable, often to an extent, where said experts are shocked to learn the metadata (which they've seen as an asset) can be changed anytime through the controller of metadata. This is in particular suprising, because there are even standards ([ERC-4906](eip-4906.md)) for notification schemes on metadata-changes and many collections have "reveal" events, where metadata is revealed only after a collection sold-out.

While we do not want to solve for this misconception, we do see a huge issue with a related common practice today. Off-chain ASSETS ("ownership over physical products", "digital collectables", "in-game assets", "responsibilities", ...) are linked to an NFT solely through metadata - hence mutable. Approaches to ensure on-chain integrity between metadata (=reference to ASSET) and a token are possible, but rarely seen.

Without ensuring integrity of metadata on-chain, we consider linking an asset through metadata very problematic. This requires trust into the metadata-controller. Further, we need to trust that the metadata provider at tokenURI is available until eternity, which has been proven otherwise (IPFS bucket disappears, central tokenURI-provider has downtimes, ...). 

When ensuring integrity of the metadata on-chain, for example through storing an (immutable) metadata-hash mapped to a token, another problem arises. For many applications, metadata should be update-able, therefore making them immutable through storing a hash is problematic. 

Finally, representing ownership of off-chain ASSETS through NFT suffers from the inhert off-chain-on-chain-integrity problem. There are approaches where off-chain ownership is enforced through having ownership over the on-chain asset. There are no approaches known, where on-chain ownership is enforced through having off-chain ownership. In the latter, dApps merely rely on some extra off-chain processes *trying* to ensure integrity, but as soon as the current owner of an NFT is incooperative or incapacitated those approaches typically fail due to lack of signing-power from the current owner of the on-chain representation.
